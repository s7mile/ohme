...
class team_model {
	...
	public function getMyTeam(){
		$sql = "SELECT t.team_name, t.team_desc, t.team_url, u.user_idx, u.team_idx, u.join_date, u.status, (SELECT COUNT(idx) FROM user_team ut where team_idx = t.idx) As c FROM team t LEFT JOIN user_team u ON t.idx=u.team_idx WHERE u.user_idx=:user_idx order by u.status asc, u.join_date asc";
	...
	public function getMemberList($team_idx){
		$sql = "SELECT t.user_idx, t.team_idx, t.join_date, t.status, u.user_id, u.user_name, u.user_profile FROM user_team t LEFT JOIN user u ON t.user_idx=u.idx WHERE t.team_idx=:team_idx order by t.status desc, t.join_date asc";
		$query = $this->db->prepare($sql);
	...
	public function inviteTeam($user_idx, $team_url, $status=0){
		...
		$sql = "INSERT INTO user_team (user_idx, team_idx, join_date, status) VALUES (:user_idx, :team_idx, :join_date, :status)";
		$query = $this->db->prepare($sql);
		$query->execute(array(':user_idx' => $user_idx, ':team_idx' => $team_idx, ':join_date' => $date, 'status' => $status));
	}
